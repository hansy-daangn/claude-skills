...
### 레이아웃
- [ ] 시안 수 = `learning Area` 화이트리스트 수 (현재 98개)
- [ ] 같은 사이즈 시안끼리 헤드 좌표가 진짜 다름 (단순 색 변형 아님, dedupe 결과 1개로 수렴 금지)
- [ ] 모든 CTA가 프레임 하단 풀와이드 bar (우측 세로 bar / 인라인 pill 금지)
- [ ] 헤드카피가 프레임 안에 완전히 들어감 (잘림 없음)
- [ ] 헤드 · CTA bar 겹침 없음 (`headMaxY = cta.y - 8` 룰)
- [ ] 로고와 헤드 겹침 없음
- [ ] 모든 텍스트 위치 `fitSize` + 절대 좌표 (text.height 사용 금지)
